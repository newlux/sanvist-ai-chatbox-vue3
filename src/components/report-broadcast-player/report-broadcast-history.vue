...
<template>
  ...
  <view class="report-broadcast-history">
    <view class="report-broadcast-history__mask" @tap="emit('close')" /><view
      class="report-broadcast-history__sheet"
    >
      <view class="report-broadcast-history__header">
        ...
        <view v-if="loading" class="report-broadcast-history__empty">
          ...
          v-else
          :key="item.bizDate"
          class="report-broadcast-history__item"
          :class="{
            'report-broadcast-history__item--selected': activeBizDate
              ? item.bizDate === activeBizDate
              : index === 0,
          }"
...
.report-broadcast-history__mask {
  position: absolute;
  inset: 0;
  background: rgb(0 0 0 / 40%);
}
.report-broadcast-history__sheet {
  position: absolute;
  right: 0;
  bottom: 0;
  left: 0;
...
.report-broadcast-history__close {
  width: 60rpx;
  height: 60rpx;
}
.report-broadcast-history__tabs {
  display: flex;
  height: 82rpx;
  gap: 12rpx;
  padding: 0 52rpx;
}
.report-broadcast-history__tab {
  display: flex;
  width: 208rpx;
  height: 82rpx;
  align-items: center;
...
.report-broadcast-history__tab--active {
  background: #191b27;
  color: #fff;
}
...
.report-broadcast-history__item {
  display: flex;
  height: 128rpx;
  flex-direction: column;
  justify-content: center;
...
.report-broadcast-history__item--selected {
  border-radius: 24rpx;
  background: #f4f3f8;
}
.report-broadcast-history__title-row {
  display: flex;
  gap: 8rpx;
  color: #1a1a1a;
  font-size: 28rpx;
...
.report-broadcast-history__new {
  color: #fe0000;
  font-size: 22rpx;
}
.report-broadcast-history__date,
.report-broadcast-history__empty {
  color: #999;
  font-size: 24rpx;
}
.report-broadcast-history__empty {
...
.report-broadcast-history__safe-area {
  flex: 0 0 auto;
}
</style>
