...
const selectedMessages = computed(() => {
  ...
  return indexes.map(idx => list[idx]).filter(Boolean).map((message) => {
    ...
    const blocks = rawBlocks.flatMap(block =>
      ...
      // guide-step / guide-suggestion 是「选一下继续问」的交互入口、guide-check 是独立于气泡的核对卡，海报里都不还原
      block => !["think", "tool_call", "status", "suggestion", "guide-step", "guide-suggestion", "guide-check"].includes(block.type),
    );
